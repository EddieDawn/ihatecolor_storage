...
현재 `src/data/photos/`에는 위 조건을 검증하기 위한 12개 모크 작품이 있다. 기존 프로토타입 사진의 복사본과 개발 fixture 전용 극단 비율 이미지 2개를 사용하며, 실제 포트폴리오 콘텐츠로 취급하지 않는다.

빈 목록과 한 장 목록은 별도 데이터 파일을 중복해 만들지 않고 `src/lib/content/photo-scenarios.ts`에서 재현한다. 페이지당 카드 수는 아직 확정되지 않았으므로 페이지네이션 경계 시나리오는 호출 시 전달한 `pageSize + 1`개를 반환한다.
